...
def test_min_entry_price_blocks_low_ask(tmp_path: Path) -> None:
    """Min price gate abstains when our side's ask is below the floor."""
    engine = QuantScoringEngine(_settings(
        tmp_path,
        quant_min_entry_price=0.20,
        quant_slippage_baseline_bps=0.0,
        quant_slippage_spread_coef=0.0,
        fee_bps=0.0,
    ))
    # BTC trending DOWN → model picks NO. ask_no=0.08 < 0.20 floor → blocked.
    packet = _packet(btc_log_return_15m=-0.01, realized_vol_30m=0.001,
                     ask_yes=0.95, ask_no=0.08, bid_yes=0.94, bid_no=0.05,
                     seconds_to_expiry=600)
    a = engine.score_market(packet)
    assert a.suggested_side == SuggestedSide.ABSTAIN
    assert any("Min price" in r for r in a.reasons_to_abstain)


def test_min_entry_price_allows_ask_at_floor(tmp_path: Path) -> None:
    """Min price gate allows trades at or above the floor price."""
    engine = QuantScoringEngine(_settings(
        tmp_path,
        quant_min_entry_price=0.20,
        quant_slippage_baseline_bps=0.0,
    ...
    # ask_no=0.25 > 0.20 floor → gate does not block (only edge/confidence limits apply).
    packet = _packet(btc_log_return_15m=-0.01, realized_vol_30m=0.001,
                     ask_yes=0.78, ask_no=0.25, bid_yes=0.76, bid_no=0.23,
                     seconds_to_expiry=600)
    a = engine.score_market(packet)
    assert not any("Min price" in r for r in a.reasons_to_abstain)


def test_min_entry_price_disabled_when_zero(tmp_path: Path) -> None:
    """Min price gate is skipped when floor is 0 (default off)."""
    engine = QuantScoringEngine(_settings(
        tmp_path,
        quant_min_entry_price=0.0,
        quant_slippage_baseline_bps=0.0,
